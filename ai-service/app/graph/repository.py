...
class GraphRepository:
    ...
    @staticmethod
    async def get_full_graph(limit: int = 500) -> dict:
        """查询全图数据，供前端可视化看板使用

        返回 vis.js 格式的 nodes 和 edges
        """
        # 查询所有节点
        node_query = """
        MATCH (n)
        ...
        LIMIT $limit
        """
        node_result = await neo4j_manager.execute_query(node_query, {"limit": limit})
        ...
        for r in node_result:
            ...
            # 统计
            stats_by_label[label] = stats_by_label.get(label, 0) + 1

            # 节点显示名称
            display_name = props.get("name") or props.get("team_name") or props.get("title") or label
            nodes.append({
                "id": node_id,
                "label": display_name,
                "group": label,
        ...
        RETURN id(a) AS from_id, id(b) AS to_id, type(r) AS rel_type, properties(r) AS rel_props
        """
        node_ids = [n["id"] for n in nodes]
        edge_result = await neo4j_manager.execute_query(edge_query, {"node_ids": node_ids})
        ...
        for r in edge_result:
            edges.append({
                ...
                "to": r["to_id"],
                "label": r["rel_type"],
                "title": json.dumps(r["rel_props"] or {}, ensure_ascii=False),
                "arrows": "to",
            })
    ...
    async def get_dispatch_triples(disaster_name: str) -> dict:
        ...
        OPTIONAL MATCH (team:救援队伍)-[:临近]->(tr:道路)-[:连通*0..6]->(mid1:道路)-[:服务]->(disaster)

        // 仓库→灾区：仓库临近的路 通过 连通 到达 服务灾区的路
        OPTIONAL MATCH (wh:物资仓库)-[:临近]->(wr:道路)-[:连通*0..6]->(mid2:道路)-[:服务]->(disaster)

        // 灾区→避难所：灾区临近的路 通过 连通 到达 服务避难所的路
        OPTIONAL MATCH (disaster)-[:临近]->(sr:道路)-[:连通*0..6]->(mid3:道路)-[:服务]->(shelter:避难场所)
        ...
        RETURN root.name AS subject,
               ...
               coalesce(child.name, child.value, '<未命名>') AS object,
               labels(child)[0] AS object_type,
               properties(child) AS object_props,
               collect({
                   predicate: type(rel2),
                   object: coalesce(grandchild.name, grandchild.value, '<未命名>'),
        ...
        if all_entity_names:
            ...
            for r in sub_result:
                ...
                for gc in r["grandchildren"]:
                    if gc["predicate"] and gc["object"] and gc["object"] != "<未命名>":
                        sub_triples.append({
                            "subject": r["object"],
